...
class AutoSizingFlowLayoutDemoController: UIViewController {
    ...
    private var items: [FlowLayoutItem] = [
        ...
        FlowLayoutItem(
            title: "动态内容",
            description: "内容可以动态变化，布局会自动重新计算。",
            imageName: "photo",
            height: 120
        ),
        FlowLayoutItem(
    ...
    private func setupCollectionView() {
        ...
        flowLayout.estimatedItemSize = CGSize(width: 100, height: 200)
...
extension AutoSizingFlowLayoutDemoController: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        let availableWidth = max(collectionView.bounds.width - padding * 2 - spacing, 100) // 确保最小宽度
        let itemWidth = availableWidth / 2
        
        // 使用预估高度而不是automaticSize.height
        return CGSize(width: itemWidth, height: 200) // 设置一个合理的预估高度
    }
} 
