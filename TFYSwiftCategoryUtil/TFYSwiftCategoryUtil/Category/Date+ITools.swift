...
public extension TFY where Base == Date {
    ...
    static func timestampToFormatterDate(timestamp: String) -> Date {
        ...
        guard let timestampDouble = timestamp.tfy.toDouble() else {
            #if DEBUG
            fatalError("时间戳位有问题")
            #else
            return Date()
...
public extension TFY where Base == Date {
    ...
    var isToday: Bool {
        let date = Date()
        if self.base.tfy.toformatterTimeString(formatter: "yyyyMMdd") == date.tfy.toformatterTimeString(formatter: "yyyyMMdd") {
            return true
        }
        return false
    }
