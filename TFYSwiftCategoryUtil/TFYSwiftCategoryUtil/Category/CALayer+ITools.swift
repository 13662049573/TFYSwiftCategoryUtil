...
// MARK:- 一、自定义链式编程
public extension TFY where Base == CALayer {
    ...
    func backgroundColor(_ hex: String) -> Self {
        base.backgroundColor = UIColor.tfy.hexStringColor(hexString: hex).cgColor
        return self
    }
    ...
    @discardableResult
    func addSublayer(_ superView: UIView) -> Self {
    ...
    @discardableResult
    func addSublayer(_ superLayer: CALayer) -> Self {
...
public extension TFY where Base: CALayer {
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func addKeyframeAnimationRotation(values: [Any] = [CGFloat(-5).tfy.degreesToRadians(), CGFloat(5).tfy.degreesToRadians(), CGFloat(-5).tfy.degreesToRadians()],
                                      keyTimes: [NSNumber]?,
