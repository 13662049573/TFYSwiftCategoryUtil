...
public extension TFY where Base: FileManager {
    ...
    /// - Returns: 返回获取的图片
    private static func getVideoImage(videoUrlSouceType: VideoUrlType = .local, path: String, seconds: Double = 1, preferredTimescale: CMTimeScale = 10, maximumSize: CGSize?, preferredTrackTransform: Bool = true) -> UIImage? {
        var videoURL: URL?
