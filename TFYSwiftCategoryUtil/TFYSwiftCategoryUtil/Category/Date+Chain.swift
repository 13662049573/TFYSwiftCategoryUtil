...
/// 时间戳的类型
public enum TFYTimestampType: Int {
    /// 秒
    case second
    /// 毫秒
    case millisecond
...
public extension TFY where Base == Date {
    ...
    /// 从 Date 获取年份
    var year: Int {
        return Calendar.current.component(Calendar.Component.year, from: base)
    }
    
    // MARK: 1.5、从 Date 获取月份
    /// 从 Date 获取年份
    var month: Int {
        return Calendar.current.component(Calendar.Component.month, from: base)
    }
...
public extension TFY where Base == Date {
    ...
    static func timestampToFormatterDate(timestamp: String) -> Date {
        ...
        guard let timestampInt = timestamp.tfy.toInt() else {
            #if DEBUG
            fatalError("时间戳位有问题")
            #else
            return Date()
    ...
    func toformatterTimeString(formatter: String = "yyyy-MM-dd HH:mm:ss") -> String {
        ...
        forma.timeZone = TimeZone.autoupdatingCurrent
        forma.dateFormat = formatter
        return forma.string(from: self.base)
    }
