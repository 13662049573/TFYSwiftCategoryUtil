...
public extension TFY where Base == UIPanGestureRecognizer {
    ...
    /// 设置最少触摸次数（>=1）
    @discardableResult
    func minimumNumberOfTouches(_ minNumber: Int) -> Self {
        base.minimumNumberOfTouches = max(1, minNumber)
        return self
    }
    
    /// 设置最大触摸次数（>=1）
    @discardableResult
    func maximumNumberOfTouches(_ maxNumber: Int) -> Self {
        base.maximumNumberOfTouches = max(1, maxNumber)
        return self
    }

    /// 设置平移位置
    @discardableResult
    func translation(_ point: CGPoint, view: UIView?) -> Self {
